...
io.on("connection", (socket) => { // Event listener for new socket connections((
    ...
    socket.on("price", (payload) => { // Event listener for incoming "message" events
        io.except(socket.id).emit('new-price', payload); // Emit a "new-message" event to all clients except the sender
    });
});
